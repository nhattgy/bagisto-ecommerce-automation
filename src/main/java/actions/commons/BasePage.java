...
public class BasePage {
    ...
    // Lấy cookies sau khi login thành công
    public Set<Cookie> getCookies(WebDriver driver) {
        return driver.manage().getCookies();
    }


    // Set cookies vào browser để login nhanh
 ...
 public void selectItemInCustomDropdown(WebDriver driver,
    ...
    public boolean isControlUnDisplayed(WebDriver driver, String locator) {
        ...
        if (elements.size() == 0) {
            // Không có element trong DOM → coi như "undisplayed"
            return true;
        } else {
            // Có trong DOM → kiểm tra có hiển thị không
            return !elements.get(0).isDisplayed();
        }
    }

    public boolean isControlUnDisplayed(WebDriver driver, String locator, String... restParameter) {
        List<WebElement> elements = getElements(driver, castParameter(locator, restParameter));
        if (elements.size() == 0) {
            // Không có element trong DOM → coi như "undisplayed"
            return true;
        } else {
    ...
    public void hoverMouseToElement(WebDriver driver, String locator, String... restParameter) {
    ...
    public void waitForElementInVisible(WebDriver driver, String locator, String... restParameter) {
        new WebDriverWait(driver, Duration.ofSeconds(GlobalConstants.LONG_TIMEOUT)).until(ExpectedConditions.invisibilityOfElementLocated(getLocator(castParameter(locator, restParameter))));
    }
    ...
    public void waitForUrlToBe(WebDriver driver, String expectedUrl) {
    ...
    public List<WebElement> waitForListElementPresence(WebDriver driver, String locator) {
        new WebDriverWait(driver, Duration.ofSeconds(GlobalConstants.LONG_TIMEOUT)).until(ExpectedConditions.presenceOfAllElementsLocatedBy(getLocator(locator)));
